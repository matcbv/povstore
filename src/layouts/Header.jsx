		...
		<header className="w-full flex items-center justify-between px-12 h-20 bg-black sticky top-0 z-10">
			...
			<nav className="flex items-center">
				<ul className="flex gap-x-6 items-center">
					<li>
						{userState.uid ? (
							<Link className="text-white font-bold" to="/account/dashboard">
								Minha conta
							</Link>
						) : (
					...
					<li>
						<Link to="/bag" className="flex items-end relative">
							...
							{checkoutState.totalQuantity > 0 && (
								<span className="absolute top-2/3 left-1/2 -translate-x-1/2 -translate-y-1/2 text-sm text-white font-bold">
									{checkoutState.totalQuantity}
								</span>
							)}
						</Link>
					</li>
