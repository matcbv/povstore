		...
		<div className="flex items-center gap-x-10 font-bold">
			...
			<div className="flex flex-col items-start gap-y-5">
				<div className="flex flex-col gap-y-2">
					<p>
						<span className="underline decoration-2 decoration-red-600 underline-offset-4">
							Código do pedido
						</span>
						: {order.id.slice(-6)}
					</p>
					...
					<p>
						Valor do pedido: R$ {order.finalPrice.toFixed(2).replace('.', ',')}
					</p>
					<p>Status: {order.status}</p>
				</div>
				<Link
					to={`/account/orders/${order.id}`}
					className="bg-black rounded-md px-4 py-3 text-xs font-bold text-white"
				>
