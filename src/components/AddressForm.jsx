...
export function AddressForm({visibilityState, addressData}){
    ...
    const handleSubmit = async (e) => {
        ...
        if(Object.keys(address).every(key => address[key] || key === 'complement')){
            ...
            try{
                ...
                const completeAddress = {...address, isDefault: addressesSnap.empty, id: addressesRef.id, addedAt: (new Date).toLocaleString('pt-BR')};
    ...
    const updateAddress = async () => {
        try{
            const addressRef = doc(db, 'users', userState.uid, 'addresses', addressData.id);
            await updateDoc(addressRef, address);
            addressDispatch({ type: actionTypes.UPDATE_ADDRESS, payload: {...address, id: addressData.id} });
            addressData.isDefault && addressDispatch({ type: actionTypes.SET_DEFAULT_ADDRESS, payload: {...address, id: addressData.id} });
            navigate('/account');
                ...
                <div className="flex flex-col gap-y-10">
                    {Object.keys(address).map(key => (
                        <div key={key} className="flex">
                            <label htmlFor={key} className="border-b-2 border-black">{labelMap[key]}</label>
                            <input type="text" name={key} value={address[key]} placeholder={key === 'complement' ? 'Opcional': ''} onChange={ handleChange } className="border-b-2 border-black bg-transparent pl-6 focus:outline-none w-full" />
                        </div>
                    ))}
                </div>
                <input type="submit" value="Salvar endereço" className="w-40 bg-black rounded-md py-3 text-sm font-bold text-white" />
            </form>
            <img
