...
export function DropdownMenu({ setDropdown }) {
	...
	return (
		<nav
			className="flex justify-end absolute left-0 top-full w-full bg-black/90 backdrop-blur-md h-80"
		...
		>
			<div className="flex justify-center w-full">
				<div
					...
					onMouseLeave={() =>
						setActiveOptions({ gender: null, category: null })
					}
				>
					<ul className="flex flex-col gap-y-10 text-white text-xl">
						{Object.entries(gendersMap).map(([gender, slug]) => (
							<li
								...
								onMouseEnter={() =>
									setActiveOptions({ ...activeOptions, gender: slug })
								}
								style={{
									visibility:
										activeOptions.gender && activeOptions.gender !== slug
											? 'hidden'
											: 'visible',
								}}
							>
								...
								<img
									src="/assets/images/catalog_right_arrow.png"
					...
					{activeOptions.gender && (
						<div
							className="flex gap-x-10 relative left-0"
							onMouseLeave={() =>
								setActiveOptions({ ...activeOptions, gender: null })
							}
						>
							<ul className="flex flex-col justify-center gap-y-10 text-white text-lg">
								{Object.entries(categoriesMap).map(([category, slug]) => (
									<li
										...
										onMouseEnter={() =>
											setActiveOptions({ ...activeOptions, category: slug })
										}
									>
										<Link to={getPath(activeOptions.gender, slug)}>
											{category}
										</Link>
										<img
											src="/assets/images/catalog_right_arrow.png"
							...
							{activeOptions.category && (
								<ul className="flex flex-col justify-center gap-y-5 w-40 text-white text-md">
									...
									{Object.entries(
										catalogMap[activeOptions.gender][activeOptions.category],
									).map(([name, slug]) => (
										<li
											key={slug}
											className="product-item cursor-pointer hover:font-bold"
										>
											<Link
												to={getPath(
													activeOptions.gender,
