			...
			if (userState.uid) {
				try {
					const userFavorites = await getDocs(
						collection(db, 'users', userState.uid, 'favorites'),
					);
					userFavorites.docs.map((fav) =>
						setFavorites((prevFavorites) => ({
							...prevFavorites,
							[fav.id]: true,
						})),
		...
		return (
			<div className="w-full h-full flex flex-col items-center justify-center gap-y-10">
				...
				<p className="font-bold text-2xl">
					Carregando produtos<span className="text-red-600">...</span>
				</p>
			</div>
		);
	}
			...
			return (
				<div className="flex flex-col items-start gap-y-5">
					<h2 className="text-2xl font-bold">
						Ainda não há produtos nessa seção
							...
							<div className="flex flex-col items-center gap-y-2">
								...
								<span className="flex flex-col items-center">
									...
									<p className="text-sm text-neutral-400">
										6x de R$
										{(Number.parseFloat(prod.price) / 6)
											.toFixed(2)
											.replace('.', ',')}
									</p>
								</span>
