...
export function OrdersDetails() {
	...
	useEffect(() => {
		setCurrentOrder(
			orderState.orders.find((order) => order.id === params.orderId),
		);
						...
						{currentOrder?.items.map((item) => (
							<div key={item.id} className="flex gap-x-10 items-center">
								...
								<div className="flex flex-col gap-y-3">
									<p className="underline underline-offset-4 decoration-red-600 font-bold">
										{item.name}
									</p>
					...
					</div>
					{
						<div key={currentOrder?.id} className="flex flex-col gap-y-10">
							<div className="flex flex-col gap-y-5">
								<h2 className="underline underline-offset-4 decoration-2 decoration-red-600 text-xl font-bold">
									Resumo da compra
								</h2>
								<div className="flex flex-col gap-y-2">
									<p>
										Data da compra:{' '}
										{new Date(currentOrder?.orderData).toLocaleDateString(
											'pt-BR',
										)}
									</p>
									...
									<p>
										Preço final: R${' '}
										{currentOrder?.finalPrice.toFixed(2).replace('.', ',')}
									</p>
								</div>
							</div>
							<div className="flex flex-col gap-y-5">
