...
export function Checkout() {
	...
	const finalizeOrder = async () => {
		...
		orderDispatch({
			...
			payload: { ...orderData, id: orderRef.id },
		});
		checkoutDispatch({ type: checkoutActionTypes.RESET });
		const checkoutSnaps = await getDocs(
			collection(db, 'users', userState.uid, 'checkout'),
		);
		...
		<main className="h-screen flex flex-col gap-y-20 my-20 mx-10 lg:mx-40">
			<section>
				<div className="w-full flex justify-between font-bold pb-20">
					<h1 className="text-2xl md:text-3xl underline decoration-red-600 underline-offset-4">
						Checkout
					</h1>
					<span
				...
				<div className="flex justify-center gap-x-20">
					<div className="flex flex-col items-start gap-y-10">
						...
						<Link
							to="/account/edit"
							className="border-2 rounded-md px-4 py-2 text-sm border-black hover:bg-black hover:text-white transition-colors"
						>
					...
					<div className=" flex flex-col gap-y-10">
						...
						<div className="h-[465px] flex flex-col items-center gap-y-5 p-2 pb-1 overflow-y-scroll custom-scrollbar">
							<CheckoutCardList
								currentPaymentState={[currentPayment, setCurrentPayment]}
							/>
						</div>
					</div>
					<div className="flex flex-col gap-y-10">
