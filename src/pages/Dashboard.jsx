...
export function Dashboard() {
		...
		return (
			<div className="w-screen h-screen flex flex-col items-center justify-center gap-y-10">
				...
				<p className="font-bold text-2xl">
					Carregando dados<span className="text-red-600">...</span>
				</p>
			</div>
		);
	}
	...
	return (
		<main className="min-h-screen flex flex-col m-20">
			...
			<section className="flex flex-col lg:flex-row items-center lg:items-stretch gap-y-20 lg:gap-y-0 justify-center md:gap-x-10 lg:gap-x-20 text-black">
				<div className="flex flex-col gap-y-14">
					<div className="flex flex-col gap-y-5">
						<h2 className="text-lg md:text-xl font-bold">
							Endereço principal<span className="text-red-600">:</span>
						</h2>
						{Object.keys(addressState.defaultAddress).length > 0 ? (
							<AddressResume address={addressState.defaultAddress} />
						) : (
							<div className="flex flex-col items-start gap-y-5">
								<p className="md:text-lg">
									Nenhum endereço definido como principal
				...
				<span className="border border-red-600 w-full lg:w-auto" />
				<div className="flex flex-col gap-y-20">
					<h2 className="text-xl md:text-3xl font-bold">
						Últimos pedidos<span className="text-red-600">:</span>
					</h2>
					{orderState.orders.length > 0 ? (
						orderState.orders
							.slice(0, 2)
							.map((order) => <OrderResume key={order.id} order={order} />)
					) : (
						<div className="flex flex-col items-start gap-y-4">
							<p className="md:text-lg">Não há pedidos feitos recentemente.</p>
							<Link
								to="/catalog"
								className="bg-black rounded-md px-8 py-3 text-sm font-bold text-white hover:scale-105 transition-transform"
							>
								Ir ao catálogo
							</Link>
						</div>
